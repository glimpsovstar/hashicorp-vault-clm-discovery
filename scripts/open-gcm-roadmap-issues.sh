...
create "M1: Secure the control plane (auth/RBAC/audit/AppRole)" \
  "$ROOT/docs/superpowers/issues/m1-control-plane-security.md" enhancement
create "M2: Durable lifecycle jobs + wire verification" \
  "$ROOT/docs/superpowers/issues/m2-durable-lifecycle-jobs.md" enhancement
create "M4: Durable Postgres scan queue (SKIP LOCKED)" \
  "$ROOT/docs/superpowers/issues/m4-durable-scan-queue.md" enhancement
create "M3: Explainable posture (findings, risk_score, waivers, PQC tags)" \
  "$ROOT/docs/superpowers/issues/m3-explainable-posture.md" enhancement
create "M5: Broader integrations (events, revoke-via-AAP, ITSM, cloud collectors)" \
  "$ROOT/docs/superpowers/issues/m5-broader-integrations.md" enhancement
